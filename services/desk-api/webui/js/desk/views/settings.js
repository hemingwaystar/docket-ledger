/* ==========================================================================
   ...
   stSwatches/stateColorSet/stateDescSet/stPalHex (state decor, 0027; palette
   pills removed — RGB color picker only) ·
  ...
  m.innerHTML = `
    ...
    <div class="modal-body">
      ...
      <div class="field" style="margin-top:8px"><label>Color</label>
        ...
        <div id="stPal" style="display:flex;gap:5px;flex-wrap:wrap;margin-top:4px;align-items:center">
          <input type="color" value="#7a8a99" title="pick a color" onchange="stPalHex(this.value)" style="width:28px;height:24px;padding:0;border:none;background:none;cursor:pointer">
        </div><div class="mini muted" style="margin-top:4px">Leave it untouched for the default chip style. Recolor any time from the state's row.</div></div>
...
/* the row's color control: just the free RGB square (11b — <input type=color>,
   zero dependencies) + a ↺ default affordance when the state carries custom
   decor. The preset palette pills were removed (user request) — the setter and
   mapIn still accept/render legacy palette-token colors, they're just no longer
   offered here. */
const stSwatches = s =>
  `<input type="color" value="${s.hex||'#7a8a99'}" title="pick a color" onchange="stateColorSet('${jsq(s.id)}',this.value)" style="width:28px;height:24px;padding:0;border:none;background:none;cursor:pointer;vertical-align:middle${s.hex?';outline:2px solid var(--brand);outline-offset:1px;border-radius:4px':''}">`
...
/* the add-state modal's color square (11b) — writes the picked hex into the
   hidden #stColor that saveState reads (palette pills were removed) */
function stPalHex(v){
...
/* priority decor (0028) — the state-decor pattern verbatim: the RGB square
   + ↺ when custom; immediate diff-guarded PATCH {color}; pid-gated. Default
   flag = the rank-derived class. (Preset palette pills removed — user request.) */
const prioDefCls = p => 'p'+Math.min(4,Math.max(1,Number(p.rank||p.id)||1));
const prioSwatches = p =>
  `<input type="color" value="${p.hex||'#7a8a99'}" title="pick a color" onchange="prioColorSet(${p.id},this.value)" style="width:28px;height:24px;padding:0;border:none;background:none;cursor:pointer;vertical-align:middle${p.hex?';outline:2px solid var(--brand);outline-offset:1px;border-radius:4px':''}">`

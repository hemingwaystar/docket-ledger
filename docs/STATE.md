...
- [ ] **Build 15a — bulk add tech:** select 2+ tickets in the queue → the bulk bar's "Add tech…" dropdown (next to "Assign to…") → pick a tech → they're added as an assignee to every selected ticket (owner untouched); the selection STAYS so you can pick another tech; re-picking the same tech is a no-op; locked-project/merged tickets are skipped.

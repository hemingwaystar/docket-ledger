...
| 49 | (caught by the Build-27b prod deploy, 2026-08-20) migrate exit 3 on 0038: seeding the client-wide billable veto row hit `null value in column "activity_type_id" … violates not-null constraint` — the documented "NULL = client-wide override" row shape STILL wasn't storable, nine months after 0017 "fixed" it | 0017 (bug #24) dropped client_rates' composite PK and added the two partial unique indexes — but dropping a PRIMARY KEY does not drop the implicit NOT NULL it stamped on its columns (attnotnull survives the constraint). Nothing noticed for 21 migrations because NO code path ever INSERTED a wide row: put_client_rate's wide branch has been latently broken in prod the entire time — it just was never called with a client-wide override | 0038 (failed atomically, recorded nowhere, so legally amended in place) now leads with `ALTER COLUMN activity_type_id DROP NOT NULL` — idempotent, completes 0017's half-fix; re-run migrate applies clean and the wide lane finally accepts both the veto seed and client-wide rate overrides | Dropping a PK leaves its implicit NOT NULLs behind — check attnotnull, not just the constraint list; and a schema "fix" that enables a new row shape is UNVERIFIED until something writes one — insert a row of the new shape in the same migration, or the first real writer becomes your test in production |
...
- [ ] **Build 27e — verify:** (1) Search boxes across Assets no longer overlap the magnifier icon (padding restored). (2) LICENCE COSTS: the Cost/term column shows the POOL (e.g. "$120.00" with "25 × $4.80/seat" beneath); annualised, Overview spend, Reports rollups and CSVs all multiply by seats; the modal field reads "Cost per seat / term". (3) Every list page has advanced filters — Licenses (client incl. MSP-wide, recurring/one-off, at-capacity/≥90%, term-end window), Contracts (client, kind, recurring, term-end window), Audit (actor multi-select) — chips, empty = no filter, clear button, live match count, CSVs honor the filtered rows. (4) NEW **Clients** nav item: list of every client with asset/licence/contract counts, needs-attention chip (id-matched — a client named as a PREFIX of another must show 0, not the other's issues), book value + annualised spend (a_see_costs); click → per-client page with scoped Assets/Licences/Contracts sections and Add buttons that open the modals PRE-FILLED with that client; MSP-wide pools appear as a footnote, never in a client's counts. (5) Contract modal's covered-assets picker lists ONLY the chosen client's CIs on first open AND on reopen after a different client (stale-DOM regression); the server also refuses cross-client coverage links now (other-client ids silently skipped, like inactive assignees). (6) Editing a record whose client was archived still SHOWS the client ("(archived)" suffix) instead of a blank picker.
- [ ] **BUILD 27d — no migration.** assets-api rebuilds (ships bootstrap.py + webui). RELOAD APP TABS (SPA staleness — a cached overview.js reproduced the row-13 lesson during verification).
- [ ] **Build 27d — advanced filtering + recurring display:** (1) Assets page has a second filter row — Client and Type multi-selects (empty = no filter, chips with ×), a Warranty-health select (active / expiring ≤ lead / expired / none on record), a purchased From–To window, ✕ Clear filters, and a live "N matches" count; Export CSV exports exactly the filtered rows. (2) Reports page gets the same filter contract (client/type/status + purchased window); every tile, the per-client rollup (assets · book value · licences/yr · contracts/yr · total/yr) and Fleet-by-type recompute from the filtered set; its Export CSV ships the same rollup (a_export_csv). MSP-wide licence pools drop out while a client filter is set (bannered). (3) REGRESSION FIX: the licence modal's client picker now lists ALL clients on open (the prefilled "All clients (MSP-wide)" label was filtering the list down to itself) — every combo suite-wide in Assets now opens unfiltered and filters as you type. (4) RECURRING TERMS: a ↻ recurring licence/contract whose anchor date has passed shows "renews <date>" (the CURRENT term's end, rolled server-side) — never red "expired"; red expiry stays for non-recurring lapses; Overview's "Coverage ending soonest" lists non-recurring items only. Build 30's worker will advance the anchor authoritatively when it posts each renewal charge.
- [ ] **BUILD 27c — no migration.** ledger-api rebuilds (ships admin.py/bootstrap.py). RELOAD APP TABS. **ROLE RE-GRANT NOTE:** Ledger admin endpoints now gate STRICTLY on their catalog ids — config + Odoo secret need `l_manage_settings` (closes the l_settings-typo hole where `l_export` alone opened them); client billing/rates/access/default-opt-in need `l_manage_clients`; activity types + type rates need `l_manage_types` (global default rates accept either manage_types or manage_settings — the control lives on the Settings page); `l_approve`/`l_export` gate ONLY approvals/exports again. Core presets are fine (only Admin holds these), but any CUSTOM role that edited rates/types via l_approve/l_export needs the matching manage box ticked in Directory → Roles (applies at next sign-in). Also: the Ledger bootstrap now ships the audit tail ONLY to sessions holding `l_view_audit` — others get an empty list (tab already hidden).
- [ ] **Build 27c — verify:** as a role with only l_export: exporting periods still works, but PUT /api/config/ledger, the Odoo-secret rotate, client rate edits and type edits all 403 ("Your role lacks: …"). As a role with l_manage_clients only: client cycle/billable/rates/access edit fine; type edits 403. A role without l_view_audit gets an empty Audit tab data-wise (bootstrap audit:[]), and the tab stays hidden.
- [ ] **BUILD 27b — migration 0038** (`apply 0038_client_billable_default.sql`, no ERROR lines: re-issues ledger.priced() with the client-wide billable veto rung + seeds dated wide client_rates rows for clients already unticked; ledger-api rebuilds after — ships admin.py/bootstrap.py + webui). RELOAD APP TABS.
- [ ] **Build 27b — "Billable by default" is real:** on a Ledger client card, untick "new time bills" → time logged on that client FROM TODAY prices at $0/non-billable everywhere (timesheets, period cards, export preview) while yesterday's time is untouched; a per-type "Billable (this client)" override set to billable STILL bills that one type (the card's per-type toggles outrank the client flag); re-ticking restores normal type-driven billing and does NOT make Internal/Cancelled types billable; the flip writes the audit log and survives reload (dated row, not just the column). Approve-locked/exported periods never change.
- [ ] **BUILD 27 — PRE-DEPLOY, one-time on the VM, BEFORE deploy.sh:** create the new secret file — `openssl rand -base64 32 | tr -d '\n' > ~/docket-ledger/secrets/pg_assets_api_password && chmod 600 ~/docket-ledger/secrets/pg_assets_api_password` — compose now lists it, so migrate and assets-api will NOT start without it. Then `scripts/deploy.sh` (migrate applies **0037**: assets schema + `assets_api` role + `a_*` permission seed + re-issued CHECKs on shared.permissions.app / audit.events.app — expect `apply 0037_assets_init.sql`, no ERROR lines; deploy.sh now also builds **assets-api**, published at :8083). Then install the UPDATED nginx conf (new `upstream assets_api` + `location /assets/`): copy, sed `__BIND_ADDR__`, `nginx -t && systemctl reload nginx`. `curl http://$BIND_ADDR:8083/readyz` → `{"ok":true,"assets":0}`. RELOAD APP TABS (suite.html gained a third tab).
- [ ] **Build 27 — the Assets app exists:** suite topbar shows **Assets — Service & Inventory**; the tab loads a native-looking app with rail Overview / Assets / Software & Licenses / Contracts & Warranties / Reports / Audit Log / Settings. Add an asset (CI tag + type + client — the intake sentinel is refused) → it lists; edit it → the change appears in the modal's **Changes** feed AND the Assets **Audit Log**, both naming you; a stale edit (two tabs) → **409 version conflict**, never a silent overwrite. Add a licence with a TYPED term (monthly / 1yr / 3yr… + ↻ recurring) → the table shows it exactly as typed, with per-term AND annualised cost; seat +/− adjusts in place; a full pool flags "at capacity" and lands in Overview's Needs attention. Add a contract, link covered CIs → the asset's detail shows its coverage. Roles: Docket → Directory → a role card now has an **Assets permissions** section (9 ids; Admin=all, Dispatcher=view/costs/CSV, Technician=view/manage-assets); a session WITHOUT `a_see_costs` sees no dollar columns anywhere; without `a_view` the app shows Overview only; a Docket **preset** apply must NOT strip a_*/l_* grants. Settings: lapse scan config (lead days / board / kinds) saves and audits. Both `/assets/` behind nginx and `:8083` direct serve the app; sign-out from Assets lands on the desk login.
- [ ] **BUILD 26 — migration 0036** (`desk.articles.deleted_at/deleted_by`, nullable, no backfill, no grant/guard change). Apply 0036 before the desk-api rebuild (ships write.py/bootstrap.py + webui). mail-worker untouched. RELOAD APP TABS.
- [ ] **Build 26 — delete a note or public reply:** open a ticket → an internal NOTE **or** a public REPLY now has a red **Delete** control (any tech who can open the ticket — no special permission). Click it → a **confirmation** prompt (naming the linked time hours if any) → on OK the note/reply is replaced by a muted **🗑 tombstone** in the thread ("Internal note / Public reply deleted by <you> · <when>"), its linked **time entry is voided** in Ledger (a voided row remains), and the deletion is written to BOTH the ticket **Audit** block and the system **Audit Log** with the ORIGINAL content, who, and when — the action **differentiates note vs reply** ("Internal note deleted" / "Public reply deleted"). A note/reply whose linked timesheet is **approved** or whose billing **period is locked** has NO Delete control and the API refuses it (**423**) — "not on immutable/locked time approvals". mail-in / system entries / auto-notes are undeletable (**409**). A locked project refuses (**423**). Deleting does NOT 409 a following property edit (version synced). Double-click / concurrent delete is idempotent (no double void/audit). Deleted content is NOT shipped to the thread (it lives only in the audit).
- [ ] **BUILD 25 — no migration.** desk-api rebuilds (ships bootstrap.py/common.py/articles.py/write.py). RELOAD APP TABS.
- [ ] **Build 25 — audit log is legible:** (1) The system **Audit Log** tab "Who" column shows the person's NAME (e.g. "HTS Admin"), not "agent:<uuid>" (the actor filter dropdown + CSV export get names too). (2) **Adding** a note/reply now records the CONTENT — the ticket Audit block and the Audit Log show "Note added — “<the note text>”" (was just "note by X"). (3) A long audit entry (a long note, or an edit's before/after) shows a **disclosure arrow (▸)**; clicking expands (▾) to the whole text; short entries have no arrow. Long unbroken strings **wrap** (no horizontal overflow in the Audit Log table). An expanded entry **stays open** across a background refresh; entries that already fit in 2 lines show **no pointless arrow**.
- [ ] **BUILD 24 — no migration.** desk-api rebuilds (ships articles.py/common.py/write.py). RELOAD APP TABS.
- [ ] **Build 24 — note additions audited + actor on every audit line + text wrap:** (1) Add an internal note or send a reply → a compact event ("Internal note added" / "Public reply sent to …" or "Public reply recorded (to …)") now appears in the ticket **Audit** block and the system Audit Log (the note/reply content still shows in the thread). Recorded-only replies (outbound off — the default) read "recorded", not "sent". (2) EVERY line in the ticket Audit block now shows the acting user + time ("Ada Lovelace · Aug 5 · 07:29"); system events read "Automation · …". (3) A note/reply containing a very long unbroken string no longer overflows the thread card horizontally, and long audit entries no longer overlap neighbouring cells — both wrap.
- [ ] **BUILD 22 — no migration.** desk-api rebuilds (ships write.py). RELOAD APP TABS. No schema/worker change.
- [ ] **Build 22 — every ticket change audits to BOTH logs + marks "Updated":** (1) Edit an internal note → it now appears in the ticket's **Audit** block ("Internal note edited — before … → after …") AND the system Audit Log, AND the ticket's "Updated" column moves / it re-sorts to the top of the queue. (2) A **property** change (state/priority/owner/group/contact/title) now shows in the ticket Audit block too (previously only the system Audit Log got it — priority & group had no ticket entry at all). (3) An **assignee** change and a **tag** change now bump "Updated" (before, they didn't move the board) and both still write to both logs. Regression check: after ANY of these, immediately change state/priority/owner → NO spurious "Version conflict" (each endpoint returns the touch-bumped version and the UI syncs it). Schedules and client-moves already did all three — confirm they still do.
- [ ] **BUILD 21 — no migration; frontend-only.** RELOAD THE APP TAB after deploying. No schema/API/worker change (adds a `ticketBlocks` per-user pref key; the prefs endpoint already stores arbitrary keys).
- [ ] **Build 21 — reorderable ticket blocks:** open a ticket → the case file is now a full-width vertical STACK of blocks in this default order: Properties, Conversation, Schedule, Audit. Each block has a slim bar with its name + ↑/↓ buttons (↑ disabled on the top block, ↓ on the bottom). The Properties block lays its fields out HORIZONTALLY (State/Priority/Owner across, etc.); Schedule and Audit rows also flow in responsive columns. Reorder with ↑/↓ → the new order sticks after a reload and is per-tech (another agent's order is unaffected). Narrow screens collapse each block back to a single column.
- [ ] **BUILD 20 — no migration; frontend-only.** RELOAD THE APP TAB after deploying. No schema/API/worker change.
- [ ] **Build 20 — ticket Audit panel:** open a ticket → the conversation thread now shows ONLY human messages (inbound email, agent replies, internal notes). The automatic system events — "Assignees: …", "Scheduled …", "Schedule for … completed", "⚡ Trigger …", state/owner/title changes, merges — are GONE from the thread and instead listed in a new **Audit** card in the right sidebar (stacked under the Schedules card), newest first, each with its timestamp. A ticket with only system events shows "No conversation yet — automatic events are in the Audit panel." in the thread. The card scrolls internally once the history gets long. (This is a pure re-home of the existing 'sys' articles — no data changes; the system audit log under Audit Log is unaffected.)
- [ ] **BUILD 19 — migrate applies 0035** (`apply 0035_schedule_completed.sql`, no ERROR lines: adds desk.ticket_schedules.completed_at/completed_by — nullable, no backfill, no new grant since desk_api already holds UPDATE on desk tables). desk-api rebuilds after (ships write.py/bootstrap.py). mail-worker UNCHANGED. Reload app tabs.
- [ ] **Build 19 — complete a schedule:** open a ticket → Schedules bar → each block now has a checkbox (shown to the block's OWN tech or anyone with 'assign'). Tick it → the block's name+time strike through, a "✓ <you> · <when>" line appears, the audit log gets one "Schedule completed" row (your name + time), and the ticket thread gets a sys article. Un-tick → strikethrough clears, "Schedule reopened" audits. On the **Schedule** calendar (Day/Week/Month) that same block now renders struck-through + dimmed. A completed FUTURE block no longer takes an on-hold ticket off hold at its start (pending_until skips completed rows); un-completing it restores the wake. A tech WITHOUT 'assign' can tick their own block but not another tech's (the API 403s). Marking complete does NOT raise a "Version conflict" on a following priority/owner/state edit (the toggle returns the fresh version and the UI syncs it).
- [ ] **BUILD 18 — no migration; frontend-only.** RELOAD THE APP TAB after deploying. Reads the existing desk.ticket_schedules (0033) — no schema, API, or worker change.
- [ ] **Build 18 — Schedule tab:** the Docket rail has a new "Schedule" item (calendar icon, between Tickets and Projects, visible to everyone). It shows a calendar of the tech time blocks that live on tickets. Day/Week/Month segmented control + ‹ Today › nav + a period label + a "N scheduled blocks" count + an "All techs" multi-select filter + a per-tech color legend. Day/Week draw a time grid: each block sits at its start time with its duration as height, overlapping blocks split side-by-side, an open-ended block draws a 1h box, today's column is tinted and a red now-line shows the current time. Month draws a day grid: each day lists up to 3 colored chips (start time + tech first name) then "+N more"; off-month days are greyed; today is outlined. Clicking any block/chip opens that ticket. A technician sees only blocks on tickets in their scope (same visibility as the queue). Empty period → a friendly empty state pointing to the ticket Schedules panel.
...
- [ ] **BUILD 17 — migrate applies 0034** (`apply 0034_article_edited.sql`, no ERROR lines: adds desk.articles.edited_at/edited_by + a ledger.period_locked() SECURITY DEFINER fn granted to desk_api; desk-api rebuilds after). Reload app tabs.
- [ ] **Build 17 — edit internal notes:** on an internal NOTE you authored (or any note if you have see_billing) → an "Edit" control opens an inline editor → change the text and/or add an attachment → Save → the note shows "(edited …)"; the audit log has one "Note edited" row with the before AND after body, your name, and the time. A REPLY / inbound mail / system note has NO Edit control (and the API refuses one). A note whose time entry is APPROVED or whose billing period is LOCKED shows no Edit and the API refuses (423). Editing a note does NOT bump the ticket's "Updated" time or re-sort it.
- [ ] **BUILD 16 — migrate applies 0033** (`apply 0033_ticket_schedules.sql`, no ERROR lines; desk-api rebuilds after — it ships write.py/bootstrap.py). mail-worker UNCHANGED. Reload app tabs post-deploy.
- [ ] **Build 16a — Schedules bar placement:** the "Schedules" bar is PERSISTENT (shows on every ticket, not just on-hold ones) and sits to the RIGHT of the props panel (layout: thread | props | schedules). The old single "Wake up" field is gone. Auto-resume still only fires once a ticket is actually on hold.
- [ ] **Build 16 — Schedules bar:** open a ticket → the "Schedules" bar is next to the props panel. Add a per-tech block (tech + start + end + note) → it lists; the ticket auto-resumes off hold at the EARLIEST future start (the worker's pending-wake). Add a second earlier block → resume moves earlier; remove the earliest → resume rolls to the next; remove all → no auto-resume. A past-only block does NOT reopen the ticket instantly. After adding/removing a schedule, immediately change the ticket's priority/owner/state → NO "Version conflict" (the schedule write returns the fresh version and the UI syncs it).
- [ ] **BUILD 15 — migrate applies 0032** (`apply 0032_ticket_assignees.sql`, no ERROR lines; desk-api rebuilds after — it ships write.py/bootstrap.py). Frontend + one migration; reload app tabs post-deploy.
- [ ] **Build 15b — bulk bar relabel + multi-assign:** the queue bulk bar's owner control now reads "Owner…" (was "Assign to…"); a NEW multi-select "Assign to…" (checkbox dropdown of active techs) + an "Assign N" button lets you tick several techs and add them to every selected ticket at once (owner untouched, existing assignees kept). Button disabled until techs are ticked; after Assign the ticket selection stays and the staged techs clear; already-assigned/locked/merged tickets are skipped. (Supersedes 15a's one-at-a-time "Add tech…".)
- [ ] **Build 15 — multiple assigned techs:** open a ticket → props panel → "Assigned techs" → add two techs via the combo → each of those techs (sign in as one, or check their queue) sees the ticket under "Mine"/"Assigned to me" AND in the dashboard tile, even if they're not the owner and not in the group; the OWNER is still a single person and unchanged. Remove a tech → they drop from their Mine views. Assignee changes never raise a "Version conflict" (side table carries no version). An archived tech that was assigned stays listed until removed; re-saving reconciles to the server's set.
- [ ] **Build 14d — default ticket group:** Settings → Queue tabs & dashboard → New tickets → "Default ticket group" select → pick Service Desk → New-ticket modal opens with Group preselected to Service Desk; "— first board —" clears it (modal falls to the first board); an archived stored group shows "(archived)" in the select and the modal falls back to the first board. Setting persists across a reload (rides desk_ui → bootstrap deskUi).
- [ ] **Build 14c — contactless ticket create:** New ticket → pick a client → set Contact to "— no contact —" (first option; a real contact stays the default) → create succeeds, the ticket opens with contact "—", and NO open email goes out (check the thread: no auto reply article) — the create trigger skips when there is no recipient; assigning a contact later via the props panel behaves as before.
- [ ] **Build 14b — wake clock:** pause a ticket → the Wake-up field is prefilled with the CURRENT local date/time (no more empty field, no more UTC-shifted display) with a Set button (Clear once a timer exists); pause then immediately set/adjust the wake time twice in a row → NO "Version conflict" alert (every ticket PATCH now refreshes the optimistic lock from the response).
